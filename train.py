...
from utils.loss_utils import l1_loss, ssim, SmothLoss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    image_weigh = viewpoint_stack[0].image_width
    row_indices = torch.arange(0, image_height).view(-1, 1).repeat(1, image_weigh).cuda() # 一个存储行号的h*w矩阵
    column_indices = torch.arange(0, image_weigh).repeat(image_height, 1).cuda()
    mask = torch.ones((1, image_height, image_weigh), dtype=torch.float32).cuda()

    smooth_loss = SmothLoss()
    ##########################################
...
##########################
        ...
        if args.binocular_consistency and iteration > args.shift_cam_start:
            ...
            focal_x, focal_y = viewpoint_cam.get_camera_focal()
